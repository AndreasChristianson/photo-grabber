...
export default async (options) => {
  // const { url, regex , suffix, saveDir} = options;

  // const response = await spinner(fetch(url), `fetching ${url}`);
  // spinner(`status: ${response.status}`)
  // const text = await spinner(response.text(), `downloading page body`);
  // spinner(`size: ${text.length} chars`)
  // const images = await spinner(() => findImages(text, regex), `finding images`);
  // spinner(`image count: ${images.length}`)
  // const imagesUrls = await spinner(() => buildImageUrls(images, suffix), `formatting image urls`);
  // imagesUrls.forEach((imageUrl) => downloadImage(imageUrl, saveDir));
    ...
    task: (context) => {
      return new Listr(
        context.imagesUrls.map((imageUrl) => ({
          ...
          task: (context) => {
            context[imageUrl] = {}
            context[imageUrl].filename = Url.parse(imageUrl).pathname.split('/').pop();
            context[imageUrl].savePath = path.join(context.saveDir, context[imageUrl].filename)
            return new Listr([{
              title: 'fetching',
              task: async (context, task) => {
              ...
              task: async (context, task) => {
                ...
                const bytes = await downloadImage(context[imageUrl].response, context[imageUrl].savePath);
